...
public class SimpleOrDefault<V,E> extends AdjacencyListGraph<V,E> {
	...
	public void addEdge(V aVertex, V otherVertex, E theEdge) {
        ...
        for (InternalEdge e : adjListSrc) {
            if (e.target.equals(otherVertex)) {
    ...
    public int getNRegular() {
        if (isDirected || acceptSelfLoop) {
            throw new RuntimeException("NO ACEPTA LOOPS, NI GRAFOS DIRIGIDOS");
        }
        ...
        for (V vertex : getAdjacencyList().keySet()) {
            int currentDegree = getAdjacencyList().get(vertex).size();

            if (toReturn == null) {
                toReturn = currentDegree;
            } else if (currentDegree != toReturn) {
                return -1;
            }
        }
    ...
    private boolean existsVertex(V source) {
        return getAdjacencyList().containsKey(source);
    }
