...
abstract public class AdjacencyListGraph<V, E> implements GraphService<V, E>{
    ...
    public boolean isBipartite(){
        ...
        // Initialize colors for all vertices with RED (unvisited)
        Map<V, Color> colors = new HashMap<>();
        for (V v : adjacencyList.keySet()) {
            colors.put(v, Color.RED);
        }

        // Start coloring vertices using BFS from each unvisited component
        // This handles disconnected graphs as well
        for(V vertex : adjacencyList.keySet()) {
            if(colors.get(vertex) == Color.RED) {
                ...
                if(!isBipartiteUtil(vertex, colors)) {
                    return false;
                }
            }
        }
    ...
    private boolean isBipartiteUtil(V vertex, Map<V, Color> colors){
        ...
        while(!queue.isEmpty()) {
            ...
            Color currentColor = colors.get(current);

            // Travel all adjacent vertices
            for(InternalEdge edge : adjacencyList.get(current)) {
                V neighbor = edge.target;
                Color neighborColor = colors.get(neighbor);

                if(neighborColor == Color.RED) {
                    // Assign opposite color to unvisited neighbor
                    colors.put(neighbor, currentColor == Color.BLUE ? Color.GREEN : Color.BLUE);
                    queue.add(neighbor);
                }
                else if(neighborColor == currentColor) {
                    // Adjacent vertices have the same color - not bipartite
                    return false;
                }
            }
        }
